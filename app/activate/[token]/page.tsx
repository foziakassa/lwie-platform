...
export default function Activate() {
  ...
  useEffect(() => {
    ...
    if (tokenFromUrl) {
      ...
      setMessage("No activation token found. Please check your email for the activation link.");
      setIsLoading(false);
    }
  }, [searchParams]);
  ...
  const activateAccount = async (token: string) => {
    try {
      ...
      if (response.ok) {
        ...
        setMessage(data.error || "Activation failed for an unknown reason."); // Handle error messages
      }
    } catch (error) {
  ...
  // Loading UI until activation attempt is resolved
  if (isLoading) {
